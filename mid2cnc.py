...
tempo=args.tempo # should be set by your MIDI...

max_active_notes = 0

def main(argv):
    global tempo
    global max_active_notes
    x=0.0
    y=0.0
    z=0.0
    ...
    for note in noteEventList:
        ...
        if last_time < note[0]:
            ...
            if max_active_notes < len(active_notes.values()):
                max_active_notes = len(active_notes.values())

            for i in range(args.offset, min(len(active_notes.values()) - args.offset, active_axes) + args.offset):

                # Which axis are should we be writing to?
                #
                j = axes_dict.get(args.axes)[i - args.offset]
    ...
    print "Max notes active: %d" % (max_active_notes)
